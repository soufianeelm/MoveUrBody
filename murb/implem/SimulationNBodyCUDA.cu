...
__global__ void kernel_naive(const dataAoS_t<float>* d, accAoS_t<float>* accelerations, const unsigned long Nbodies, const float softSquared, const float* g)
{
...
void SimulationNBodyCUDA::computeBodiesAcceleration()
{
    ...
    // flops = n² * 18
    kernel_naive<<<gridDim, blockDim>>>(d_d, d_accelerations, Nbodies, softSquared, d_g);
